...
if __name__ == "__main__":
    ...
    # Create folder
    dirname = '/Users/Andrew/OneDrive - GIST/Code/Graduation/PC_RGD_CCG_bM_MIU/export_CCG/'
    if PV_Sandia:
        dirname += 'PV_Sandia/'
    ...
    plt.xlabel('Time(h)', fontsize=FONTSIZE)
    plt.ylabel('Power(kW)', fontsize=FONTSIZE)
    plt.legend(ncol=1, loc='upper left', frameon=True, fancybox=False, edgecolor='black', framealpha=0.8, fontsize=14)
    ...
    np.savetxt('phi_bM.csv', phi_data, delimiter=',', header='phi_WT,phi_PV,phi_load,phi_curt_WT,phi_curt_PV', comments='', fmt='%.2f')